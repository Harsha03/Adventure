"""Castle Blackthorn text adventure game."""
...
def print_pause(message, delay=2):
    """Print *message* and then pause for *delay* seconds."""
    print(message)
    time.sleep(delay)


def valid_input(prompt, options):
    """Keep asking *prompt* until the user responds with one of *options*."""
...
def choose_guardian():
    """Randomly select the castle’s guardian."""
    return random.choice(["dragon", "undead knight", "cunning warlock"])


def explore_forest(inventory):
    """Let the player search the forest and (maybe) find a shield."""
...
def storm_gate(guardian, inventory):
    """Resolve the fight when the player confronts the guardian."""
...
def field(guardian, inventory):
    """Offer the player a choice of actions from the main field."""
...
def main():
    """Run the game loop and offer to replay."""
