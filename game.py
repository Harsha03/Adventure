...
def valid_input(prompt, options):
    ...
    while response not in options:
        response = input(f"Please enter {', '.join(options)}: ").lower()
    return response
...
def intro():
    ...
    print_pause("To your left is an ancient forest, rumored to hide a magical shield.")
    print_pause("Armed only with a simple torch, you feel both anxious and excited.")
...
def explore_forest(inventory):
    ...
    if "shield" in inventory:
        print_pause("You've already retrieved the magical shield here; it glints at your side.")
    else:
        print_pause("You push aside vines and find a rune-etched shield lying on a stone!")
        inventory.append("shield")
        print_pause("You strap the shield to your arm. Feels sturdy and light.")
    print_pause("You make your way back to the castle gates.")
    return inventory

def storm_gate(guardian, inventory):
    print_pause(f"You stride up to the castle gate and the {guardian} emerges!")
    print_pause("It roars challenge and prepares to strike.")
    if "shield" in inventory:
        print_pause("You raise your magical shield—its runes glow and the attack harmlessly deflects!")
        print_pause("Seizing your chance, you dash past and conquer the guardian!")
        return True
    else:
...
def field(guardian, inventory):
    ...
    if choice == "1":
        ...
        if win:
            print_pause("You’ve defeated the guardian and claimed the castle! YOU WIN!")
        else:
